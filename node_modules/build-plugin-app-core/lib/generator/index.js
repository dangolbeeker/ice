...
var Generator = /** @class */ (function () {
    function Generator(_a) {
        var _this = this;
        var rootDir = _a.rootDir, targetDir = _a.targetDir, defaultData = _a.defaultData, log = _a.log, plugins = _a.plugins, debugRuntime = _a.debugRuntime;
        this.addExport = function (registerKey, exportData) {
            var exportList = _this.contentRegistration[registerKey] || [];
            checkExportData_1.default(exportList, exportData, registerKey);
        ...
        this.removeExport = function (registerKey, removeExportName) {
            var exportList = _this.contentRegistration[registerKey] || [];
            _this.contentRegistration[registerKey] = removeExportData_1.default(exportList, removeExportName);
            if (_this.rerender) {
                _this.render();
            }
        };
        this.render = function () {
            ...
            _this.renderData.runtimeModules = getRuntimeModules_1.default(plugins, _this.targetDir, _this.debugRuntime);
            _this.renderTemplates.forEach(function (args) {
                _this.renderFile.apply(_this, args);
            });
        };
        this.debounceRender = debounce(this.render, RENDER_WAIT);
        ...
        this.addTemplateDir = function (template, extraData) {
            if (extraData === void 0) { extraData = {}; }
            var _a = typeof template === 'string' ? { templateDir: template, targetDir: '' } : template, templateDir = _a.templateDir, targetDir = _a.targetDir;
            var templates = globby.sync(['**/*'], { cwd: templateDir });
            templates.forEach(function (templateFile) {
                _this.addRenderFile(path.join(templateDir, templateFile), path.join(_this.targetDir, targetDir, templateFile), extraData);
            });
            if (_this.rerender) {
                _this.debounceRender();
            }
        ...
        this.renderFile = function (templatePath, targetPath, extraData) {
            ...
            if (path.extname(templatePath) === '.ejs') {
                ...
                var content = ejs.render(templateContent, __assign(__assign({}, _this.renderData), extraData));
                try {
    ...
    Generator.prototype.getExportStr = function (registerKey, dataKeys) {
        ...
        var _b = generateExports_1.default(exportList), importStr = _b.importStr, exportStr = _b.exportStr;
    ...
    Generator.prototype.parseRenderData = function () {
        ...
        return __assign(__assign(__assign({}, this.renderData), exportsData), { staticConfig: staticConfig.length && staticConfig[0], globalStyle: globalStyles.length && globalStyles[0], entryImportsBefore: this.generateImportStr('addEntryImports_before'), entryImportsAfter: this.generateImportStr('addEntryImports_after'), entryCodeBefore: this.contentRegistration.addEntryCode_before || '', entryCodeAfter: this.contentRegistration.addEntryCode_after || '' });
    };
