...
function walker(_a) {
    ...
    var pageFilePaths = utils_1.getPagePaths(pagesDir);
    var routesCollect = [];
    var routesMap = {};
    var layoutMap = {};
    pageFilePaths.forEach(function (pageFilePath) {
        ...
        var isLayoutLike = utils_1.isNestFileName(name);
        ...
        var dirArrUpper = dirArrFormat.map(function (s) { return utils_1.upperCaseFirst(s); }).join('');
        // handle $ syntax: dynamic path
        var dirArrHandleDynamic = (caseSensitive ? dirArrFormatSensitive : dirArrFormat).map(function (i) {
            var out = i;
            // handle dynamic path
            if (utils_1.isOptionalDynamicPath(i)) {
                out = utils_1.transformOptionalDynamicPath(i);
            }
            else if (utils_1.isDynamicPath(i)) {
                out = utils_1.transformDynamicPath(i);
            }
            return out;
        });
        // get router path
        ...
        var filePath = utils_1.formatPathForWin(path.relative(path.dirname(routesTempPath), path.join(pagesDir, pageFilePath)));
        ...
        routesCollect = nest_1.default(layoutMap, pageConfig, routesCollect);
    });
    // amend collects
    amender_1.default(rootDir, routesTempPath, routesCollect);
    // generate splices
    var routesSplices = splicer_1.default(routesCollect, routerOptions);
    // output into .tmp
