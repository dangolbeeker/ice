...
function loopAmend(payload, collect) {
    collect.forEach(function (item) {
        ...
        item.component = utils_1.transformComponentName(component);
        if (children) {
...
function addDefaultLayout(rootDir, routersTempPath, collect) {
    ...
    var layoutJSFilePaths = utils_1.getJsFilePathsIn(path.join(rootDir, 'src/layouts/index'));
    var defaultLayoutExists = Boolean(layoutJSFilePaths.length);
    var defaultLayoutPath = layoutJSFilePaths[0] || '';
    var defaultLayoutPathRelative = utils_1.formatPathForWin(path.relative(path.dirname(routersTempPath), defaultLayoutPath));
...
function addDefault404(rootDir, routersTempPath, collect) {
    ...
    var default404PathRelative = utils_1.formatPathForWin(path.relative(path.dirname(routersTempPath), default404Path));
    if (default404Exists) {
        var config404 = {
            routePath: '',
            exact: String(false),
