...
module.exports = function (_a, _b) {
    var t = _a.types;
    var routesPath = _b.routesPath, alias = _b.alias, tempDir = _b.tempDir, applyMethod = _b.applyMethod;
    return {
        visitor: {
            Program: {
                enter: function (programPath, state) {
                    programPath.traverse({
                        ImportDeclaration: function (nodePath) {
                            var isRoutesFile = routesPath.includes(state.filename);
                            if (isRoutesFile) {
                                ...
                                if (t.isImportDefaultSpecifier(specifiers[0]) && specifiers.length === 1) {
                                    if (t.isStringLiteral(source)) {
                                        ...
                                        var newValue = formatPagePath({ routesPath: state.filename, tempDir: tempDir, value: value, alias: alias, applyMethod: applyMethod });
...
function matchAliasPath(alias, value, applyMethod) {
    ...
    Object.keys(alias).forEach(function (currKey) {
        if (value.startsWith(currKey)) {
            ...
            var currAliasPath = applyMethod('formatPath', path.join.apply(path, __spreadArray([alias[currKey]], args)));
...
function formatPagePath(_a) {
    var routesPath = _a.routesPath, value = _a.value, alias = _a.alias, tempDir = _a.tempDir, applyMethod = _a.applyMethod;
    ...
    if (matchedPagePath && pagePathRegExp.test(matchedPagePath)) {
        ...
        var newValue = pageName ? tempDir + "/" + pageName + "/Layout" : '';
        return newValue;
    }
}
/**
